...
describe("resume — provider/adapter compatibility", () => {
	...
	it("fails when provider is unavailable", async () => {
		...
		if (!result.success) {
			expect(
				["provider_incompatible", "models_changed"].includes(result.reason),
			).toBe(true);
		}
	});
...
describe("resume — non-resumable states", () => {
	...
	const nonResumableStates: LifecycleState[] = [
		"complete",
		"replaced",
		"abandoned",
	];
...
describe("validateFrozenCapabilities", () => {
	it("returns null when models are still available", async () => {
		...
		const result = validateFrozenCapabilities(contract, {
			config,
			getModels: () => models,
			getProviders: () => providers,
		});
	...
	it("returns error when model is no longer in registry", async () => {
		...
		await expect(
			validateStartupContract(config, models, providers),
		).rejects.toThrow(/model.*strong.*not found/);
	});
});
