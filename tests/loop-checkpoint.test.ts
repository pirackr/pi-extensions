...
describe("research_checkpoint counts real sources from notes.md (integration)", () => {
  ...
  async function startResearch(mission: string): Promise<string> {
    await mock.commands.research.handler(`--yes --profile quick "${mission}"`, mockCtx(cwd));
  ...
  async function checkpoint(params: Record<string, unknown>): Promise<string> {
    ...
    const result = await tool.execute("test-call", params, undefined, undefined, mockCtx(cwd));
  ...
  it("applies min(reported, counted) and hints when the model over-reports", async () => {
    ...
    const text = await checkpoint({ profile: "quick", round: 1, totalSources: 10 });
    expect(text).toContain("🔴 CONTINUE");
    expect(text).toContain("min sources: 3/15"); // floor uses counted, not reported
    expect(text).toContain("⚠ reported 10 sources but notes.md lists 3 unique URLs");
  });
  ...
  it("counts zero URLs in a URL-less notes.md and still hints", async () => {
    ...
    const text = await checkpoint({ profile: "quick", round: 1, totalSources: 5 });
    expect(text).toContain("⚠ reported 5 sources but notes.md lists 0 unique URLs");
  });

  it("appends the hint to the max-rounds PROCEED verdict", async () => {
    const dir = await startResearch("test mission four");
    fs.writeFileSync(path.join(dir, "notes.md"), "- Claim → https://example.com/a\n");
    const text = await checkpoint({ profile: "quick", round: 10, totalSources: 10 });
    expect(text).toContain("🟢 PROCEED (max rounds reached)");
    expect(text).toContain("⚠ reported 10 sources but notes.md lists 1 unique URLs");
  });
