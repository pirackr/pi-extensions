...
function harness(
	...
	const pi = {
		registerFlag() {},
		getFlag(name: string) {
			return name === "web-search-max-lookups" ? options.budget : undefined;
		},
...
const nativeJson = JSON.stringify([
	{ title: "Native", url: "https://example.com/a", snippet: "Useful result" },
]);

beforeEach(() => {
	clientLookup.mockReset();
	clientLookup.mockResolvedValue({
		query: "q",
		results: [],
		engines: [],
		partialFailures: [],
...
describe("registered web_search native routing", () => {
	...
	it.each([
		...
		[
			"empty snippet",
			JSON.stringify([{ title: "x", url: "https://example.com", snippet: "" }]),
		],
		[
			"credential URL",
			JSON.stringify([
				{ title: "x", url: "https://user:pass@example.com", snippet: "s" },
			]),
		],
	...
	it.each([
		...
		"serializes a native-only forced payload for supported providers",
		async (model, tools, toolChoice) => {
			const complete = vi.fn(async (_model, _context, options) => {
				const payload = await options.onPayload({
					tools: [{ type: "function", name: "client" }],
				});
	...
	it("falls back exactly once after a redacted native failure", async () => {
		const h = harness({
			complete: vi.fn().mockRejectedValue(new Error("secret sk-live-value")),
		});
		const result = await h.tool.execute(
			"id",
			{ query: "q" },
	...
	it.each([
		...
		"uses client-only routing for unsupported providers and proxies",
		async (model) => {
	...
	it("keeps explicit engines client-only", async () => {
		...
		expect(h.complete).not.toHaveBeenCalled();
		expect(clientLookup).toHaveBeenCalledWith(
			expect.objectContaining({ engine: "exa" }),
		);
	});
	...
	it("bypasses native when advanced options are present", async () => {
		...
		await h.tool.execute(
			...
			{
				query: "q",
				advancedOptions: { exa: { includeDomains: ["example.com"] } },
			},
			undefined,
			undefined,
			h.ctx,
	...
	it("does not fallback when caller cancellation aborts native", async () => {
		...
		const h = harness({ complete });
		await expect(
			h.tool.execute("id", { query: "q" }, controller.signal, undefined, h.ctx),
		...
		await expect(
			h.tool.execute(
				"id",
				{
					query: "q",
					advancedOptions: {
						tinyfish: { recency_minutes: 5, after_date: "2026-01-01" },
					},
				},
				undefined,
				undefined,
	...
	it("applies the call budget and clamps native result count to limit", async () => {
		...
		const h = harness({
			complete: vi.fn().mockResolvedValue(response(JSON.stringify(many))),
			budget: "1",
		});
		...
		await expect(
			h.tool.execute("id2", { query: "q2" }, undefined, undefined, h.ctx),
	...
	it("falls back on native timeout without treating it as caller cancellation", async () => {
		...
		try {
			...
			expect(clientLookup).toHaveBeenCalledTimes(1);
			expect(result.details.partialFailures).toContainEqual({
				engine: "native:openai",
				error: "native search timed out",
			});
			expect(result.usage.totalTokens).toBe(18);
		} finally {
			timeout.mockRestore();
	...
	it("explains unusable native output while preserving client diagnostics and usage", async () => {
		clientLookup.mockResolvedValue({
			...
			results: [],
			engines: [],
			partialFailures: [{ engine: "tinyfish", error: "no credentials" }],
		});
