...
function buildWorkspace(
	...
	if (lifecycleState) {
		...
		persistLifecycle(
			{ path: wsPath, projectRoot, mission, runId, transitionId } as Workspace,
			snapshot,
		);
	}
...
describe("lifecycle state machine", () => {
	...
	it("validates allowed transitions from paused", () => {
		...
		expect(() =>
			transitionLifecycle(snap, "abandoned", "cancel"),
		).not.toThrow();
		expect(() =>
			transitionLifecycle(snap, "replaced", "new run"),
		).not.toThrow();
	...
	it("validates allowed transitions from failed", () => {
		...
		expect(() =>
			transitionLifecycle(snap, "abandoned", "give up"),
		).not.toThrow();
		expect(() =>
			transitionLifecycle(snap, "replaced", "new run"),
		).not.toThrow();
...
describe("/research list — workspace discovery", () => {
	...
	it("excludes hidden directories", () => {
		...
		expect(entries[0].path).toBe(
			path.join(tmpDir, ".research", "visible-workspace"),
		);
	});
	...
	it("excludes .research/cache/web/ directories", () => {
		...
		// Create cache dirs
		fs.mkdirSync(path.join(tmpDir, ".research/cache/web/cache-1"), {
			recursive: true,
		});
		fs.mkdirSync(path.join(tmpDir, ".research/cache/web/cache-2"), {
			recursive: true,
		});
		...
		expect(entries.length).toBe(1);
		expect(entries[0].path).toBe(
			path.join(tmpDir, ".research", "normal-workspace"),
		);
	});
	...
	it("reports malformed workspace (no state, no manifest)", () => {
		...
		expect(malformed[0].path).toBe(
			path.join(tmpDir, ".research", "partial-workspace"),
		);
	...
	it("reports malformed workspace with parse errors", () => {
		...
		fs.writeFileSync(
			path.join(badDir, ".research", "run.json"),
			"not json{{}",
			"utf-8",
		);
		...
		expect(malformed.length).toBe(1);
		expect(malformed[0].path).toBe(
			path.join(tmpDir, ".research", "bad-workspace"),
		);
	});
	...
	it("includes lifecycle status in entries", () => {
		...
		markAbandoned(
			{
				path: path.join(tmpDir, ".research", "abandoned-ws"),
				projectRoot: tmpDir,
				mission: "abandoned",
				runId: "run-2",
				transitionId: "tr-2",
...
describe("/research status — workspace lookup", () => {
	...
	it("looks up workspace by absolute path", () => {
		...
		const entry = lookupWorkspace(
			tmpDir,
			path.join(tmpDir, ".research", "abs-test"),
		);
		expect(entry).not.toBeNull();
...
describe("/research clear — lifecycle abandonment", () => {
	...
	it("abandons the workspace without deleting artifacts", () => {
		...
		expect(fs.existsSync(path.join(ws.path, ".research", "run.json"))).toBe(
			true,
		);
		expect(
			fs.existsSync(path.join(ws.path, ".research", "run-state.json")),
		).toBe(true);
	});
