...
describe("DuckDuckGoEngine normalized failures", () => {
	...
	it("surfaces HTTP 500 as a service error", async () => {
		...
		try {
			await expect(
				engine.search({ query: "test", limit: 3 }),
			).rejects.toThrow();
		} finally {
...
describe("webLookup routing", () => {
	...
	it("records contention skip as partialFailure in auto mode and continues", async () => {
		...
		expect(result.results[0].engine).toBe("exa");
		expect(result.engines).toEqual(["exa"]);
		const tfFailures = result.partialFailures.filter(
	...
	it("attempts keyless DuckDuckGo when other engines lack credentials", async () => {
		const ctx = makeContext({ tinyfishKey: null, exaKey: null });
		mockReserve.mockResolvedValue("allowed");
		const html = `<html><body>
      <a class="result__a" href="//duckduckgo.com/l/?uddg=https%3A%2F%2Fexample.com%2F">Example</a>
      <a class="result__snippet" href="#"><b>Snippet</b> text</a>
		...
		globalThis.fetch = async () =>
			Promise.resolve({ ok: true, text: async () => html } as Response);
		try {
			const result = await webLookup(makeRequest(), ctx);
			expect(result.engines).toEqual(["duckduckgo"]);
			expect(result.results).toHaveLength(1);
			expect(result.results[0].engine).toBe("duckduckgo");
			// tinyfish + exa skipped on credentials; ddg is NOT a credential failure
			const credentialFailures = result.partialFailures.filter(
				(pf) => pf.error === "provider credentials unavailable",
			);
			expect(credentialFailures.map((pf) => pf.engine).sort()).toEqual([
				"exa",
				"tinyfish",
			]);
			expect(
				result.partialFailures.some((pf) => pf.engine === "duckduckgo"),
			).toBe(false);
			expect(mockExaSearch).not.toHaveBeenCalled();
			expect(mockTinyFishSearchQuery).not.toHaveBeenCalled();
		} finally {
...
describe("fetchWeb orchestrator", () => {
	const makeRequest = (
		overrides: Partial<FetchWebRequest> = {},
	): FetchWebRequest => ({
		url: "https://example.com/page",
		...overrides,
	});

	const makeContext = (overrides: { tinyfishKey?: string | null } = {}) => ({
		credentials: {
			tinyfish:
				overrides.tinyfishKey !== undefined ? overrides.tinyfishKey : "tf-key",
			exa: "exa-key",
	...
	it("returns validation error without falling back to Readability", async () => {
		...
		globalThis.fetch = async () =>
			Promise.resolve({
				...
				headers: new Headers({ "content-type": "text/html" }),
				text: async () =>
					`<html><body><article><p>RB content</p></article></body></html>`,
			} as Response);
		try {
			const result = await fetchWeb(makeRequest(), makeContext());
			expect(result.strategy).toBe("none");
			...
			expect(result.attempts).toEqual([
				{
					strategy: "tinyfish",
					outcome: "failed",
					reason: "provider rejected the request (validation error)",
				},
			]);
	...
	it("stops immediately on cancellation, no retry, no fallback", async () => {
		...
		mockTinyFishFetchGetContents.mockImplementation(
			(_params: any, options: any) => {
				if (options?.signal?.aborted) {
					const err = new Error("aborted");
					(err as any).name = "AbortError";
					throw err;
